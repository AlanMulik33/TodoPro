...
const AuthProvider = ({ children }) => {
  ...
  const checkSession = useCallback(async () => {
    try {
      ...
      const { value: sessions } = await window.storage.get('db:sessions');
      ...
      const { value: users } = await window.storage.get('db:users');
...
function App() {
  ...
  useEffect(() => {
    if (isOnline && data.offlineQueue.length > 0) {
      addToast(`🔄 Menyinkronkan ${data.offlineQueue.length} perubahan...`, 'info');
      setTimeout(() => {
        clearQueue();
        addToast('✓ Sinkronisasi selesai', 'success');
      }, 1500);
    }
  }, [isOnline, data.offlineQueue.length, addToast, clearQueue]);
